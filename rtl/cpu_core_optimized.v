...
module cpu_core_optimized #(
    ...
    wire       btb_hit = btb_valid[if_id_pc[31:2] % BTB_ENTRIES];
    wire [31:0] btb_pred = btb_target[if_id_pc[31:2] % BTB_ENTRIES];
    ...
    generate
        if (USE_SHADOW_STACK) begin
            shadow_stack ss (
                .clk(clk), .rst(rst),
                .push(cfi_push), .pop(cfi_pop), .ret_addr(cfi_ret_addr),
                .enable(1'b1), .expected_ret(cfi_expected),
                .cfi_violation(cfi_violation), .depth(cfi_depth)
            );
        end
    endgenerate
        ...
        case (id_ex_opcode)
            ...
            OP_BRANCH: begin
                case (id_ex_funct3)
                    ...
                    3'b110: ex_take_branch = ($signed(fwd_rs1_a) < 0);
                    3'b111: ex_take_branch = ($signed(fwd_rs1_a) >= 0);
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (!halted) begin
                ...
                if (!if_id_valid && !load_use_hazard) begin
                    if_id_valid <= 1'b1;
                    if_id_pc <= pc;
                ...
                if (id_ex_valid && !ex_mem_valid) begin
                    ...
                    if (id_ex_opcode == OP_BRANCH || id_ex_opcode == OP_JAL || id_ex_opcode == OP_JALR) begin
                        btb_target[id_ex_pc[31:2] % BTB_ENTRIES] <= ex_next_pc;
                        btb_valid[id_ex_pc[31:2] % BTB_ENTRIES] <= 1'b1;
                    end
                end
